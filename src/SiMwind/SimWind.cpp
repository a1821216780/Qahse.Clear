...
// ───────────────────────────────── File Info ─────────────────────────────────
//
// 该文件实现了 SimWind 风场生成模块，基于 Mann 模型和 FFT 合成生成三维湍流风场，
// 支持输出 BTS、Bladed WND、TurbSim 兼容 WND 格式文件及统计摘要。
//
// ──────────────────────────────────────────────────────────────────────────────
...
{
	// ---- 原始输入 ----
	SimWindInput input;                                          ///< 原始输入参数结构体（用户/文件来源）
	// ---- 网格与时间维度 ----
 ...
 *  @endcode
 */
bool IsSteadyEwmWindModel(const SimWindInput &input)
{
 ...
 */
bool IsSyntheticStochasticModel(const SimWindInput &input);
 ...
 *  @endcode
 */
bool HasImprovedVkAtmosphericInputs(const SimWindInput &input)
{
...
/** @brief 检查指定湍流分量是否启用。
    @param input SimWindInput输入参数。
    @param comp 分量索引 (0=u, 1=v, 2=w)。
    @return 如果对应分量已启用返回true，否则返回false。
    @code
    if (ComponentEnabled(input, 0)) { u分量已启用  }
    @endcode */
bool ComponentEnabled(const SimWindInput &input, int comp)
{
	if (comp == 0)
		return input.calWu;
	if (comp == 1)
...
/** @brief 检查是否使用谱方法生成湍流。
    ...
    @endcode */
bool UsesSpectralTurbulenceGeneration(const SimWindInput &input)
{
...
/** @brief 检查是否有任何气象输入提示（理查森数、摩擦速度、z/L等）。
    ...
    @endcode */
bool HasAnyMeteorologyHints(const SimWindInput &input)
{
...
/** @brief 验证 SimWindInput 输入参数的有效性。
    @param input SimWindInput输入参数。
    ...
    @endcode */
void ValidateGenerateInput(const SimWindInput &input)
{
	if (input.gridPtsY <= 0)
...
void ValidateInput(const SimWindInput &input)
{
	switch (input.mode)
	{
	case Mode::GENERATE:
		...
		return;
	default:
		throw std::runtime_error("SimWind supports only Mode=GENERATE");
	}
}
...
/** @brief 将 DEFAULT_COH 相干模型解析为实际模型。
    ...
    @endcode */
std::array<CohModel, 3> ResolveCoherenceModels(const SimWindInput &input)
{
...
/** @brief 从 SimWindInput 构建完整的 SimWindConfig。
    @param input SimWindInput输入参数。
    ...
    @endcode */
SimWindConfig BuildConfig(const SimWindInput &input)
{
	ValidateInput(input);
 ...
 * @param input 已解析的SimWindInput配置
 * @note       直接委托给ValidateInput；无输出文件生成。
 * @code SimWind::ValidateInputOnly(input); @endcode
 */
void SimWind::ValidateInputOnly(const SimWindInput &input)
{
	ValidateInput(input);
}
 ...
 * @param input    已解析的SimWindInput配置（湍流模型、网格、文件输出开关等）
 ...
 */
SimWindResult SimWind::Generate(const SimWindInput &input, SimWindProgressCallback progress)
{
 ...
 * @note           内部依次调用ReadSimWindInput(qwdPath)和SimWind::Generate。
...
{
	return Generate(ReadSimWindInput(qwdPath), std::move(progress));
}
