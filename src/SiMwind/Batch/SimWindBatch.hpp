...
// ───────────────────────────────── File Info ─────────────────────────────────
//
// 该文件声明了 SimWindBatch 批量执行结构体（SimWindBatchCaseResult）和 SimWindBatchRunner 类，
// 支持多案例、多线程批量风场生成。
//
// ──────────────────────────────────────────────────────────────────────────────
...
struct SimWindBatchResult
{
	...
	std::vector<SimWindBatchCaseResult> cases;    ///< 各案例结果列表
};

/// @brief SimWindBatch 进度回调函数类型。
using SimWindBatchProgressCallback = std::function<void(const std::string &)>;
...
public:
	 ...
	 * @return 批量执行结果
	 */
	static SimWindBatchResult RunFromFile(const std::string &qwdPath,
