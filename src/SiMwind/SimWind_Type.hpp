...
// ───────────────────────────────── File Info ─────────────────────────────────
//
// 该文件定义 SimWind 模块的所有数据类型，包括：
// - 枚举：工作模式、湍流模型、风模型、IEC 标准参数等
// - 主输入结构体 SimWindInput（对应 .qwd 文件）
...
enum class Mode
{
	GENERATE = 0, ///< SimWind 生成风文件
	BATCH = 1     ///< SimWind 批量生成模式（从 Excel 参数表）
};
...
{
	// ---- 模式与双轴模型 ----
	Mode mode = Mode::GENERATE;                      ///< SimWind 工作模式 (GENERATE / BATCH)，对应 .qwd 关键字 Mode
