...
// ───────────────────────────────── File Info ─────────────────────────────────
//
// 该文件提供 WindL 模块的 IO 子系统，包括 YAML 序列化辅助函数、QWD 文件解析、
// WindLInput 验证和输出路径推导。
//
// ──────────────────────────────────────────────────────────────────────────────
...
{
/** @brief YAML 文档根键名称常量 "Qahse.WindL"，所有 YAML 键均以此为前缀 */
	inline constexpr const char *kYamlRoot = "Qahse.WindL";

	/**
 * @brief 构造带根键前缀的完整 YAML 键名
 * @param key 不带前缀的键名
 * @return 完整 YAML 键名，格式为 "Qahse.WindL.{key}"
 * @note 确保 YAML 输出中的键名在 Qahse.WindL 命名空间下
 */
 ...
 * @param key 原始键名
 * @return YAML 格式返回 "Qahse.WindL.key"，文本格式返回原始 key
 */
...
inline void ResolveIfSet(const std::string &baseFilePath, std::string &path)
	...
	/**
 * @brief 解析 WindLInput 中所有文件路径并确保输出目录存在
 * @param input WindLInput 结构体（输入输出参数）
 ...
 *          - \c value：数据成员的左值引用，序列化器通过此引用直接读写 WindLInput 字段
 *          配合 Field() 便捷工厂函数和 WindLInputSerializer::Fields() 折叠表达式使用，
 *          在编译期将关键字与数据成员绑定，避免运行时字符串查找开销。
 * @note 用于 WindLInputSerializer，将键名字符串与数据成员引用绑定
 */
	template <typename T>
	struct FieldRef
	{
 ...
 * @note 类型推导避免了显式指定模板参数，简化 WindLInputSerializer::Fields 调用中的字段绑定语法。
...
/**
	 * @class WindLInputSerializer
	 * @brief WindLInput 序列化器，负责 .qwd 文本和 YAML 格式的序列化/反序列化
	 *
	 * 继承自 Serializer 基类，将 WindLInput 数据作为公开成员 data 直接管理
	 ...
	 * 在 YAML 输出中，所有字段键名均通过 KeyForFormat 自动添加 "Qahse.WindL." 根键前缀。
	 */
...
/**
		 * @brief 从已有 WindLInput 数据构造序列化器，用于后续写入操作
		 * @param input 要序列化输出的 WindLInput 数据的副本
		 * @return 无（构造函数）
		 * @note 设置 ValueFirst=true 使文本输出格式为"值在前、关键字在后"，兼容 .qwd 文本格式。
...
/**
		 ...
		 *       直接绑定到 WindLInput 数据成员。普通枚举字段由 magic_enum 自动处理序列化/反序列化，
...
/**
		 ...
		 *       KeyForFormat 根据当前序列化格式适配键名：YAML 格式自动添加 "Qahse.WindL." 前缀
		 *       （如 "Mode" → "Qahse.WindL.Mode"），文本格式保持原始键名不变以匹配 .qwd 格式。
...
/**
	...
	/**
 * @brief 向 YAML 文档添加带 "Qahse.WindL." 根键前缀的字符串节点
 * @param yaml YML 对象
 * @param key 不带前缀的键名
 * @param value 字符串值
 * @note 内部调用 yaml.AddNode(WindLYamlKey(key), value)
 */
 ...
 * @param key 不带前缀的键名
 * @param value 要写入的值
 * @note 内部调用 AddYamlNode，自动添加 "Qahse.WindL." 根键前缀
 */
template <typename T>
 ...
 * @return 读取的整数值
 * @note 通过 WindLYamlKey 自动添加 "Qahse.WindL." 前缀查找
 */
 ...
 * @note 通过 WindLYamlKey 自动添加 "Qahse.WindL." 前缀
...
/**
 * @brief 从 .qwd 或 .yml 文件读取 WindLInput 主输入数据
 * @param path 输入文件路径（.qwd 为文本关键字-值格式，.yml/.yaml 为 YAML 格式）
 * @return 解析得到的 WindLInput 结构体，包含所有风场模拟参数
 * @note 自动根据扩展名选择解析器：YAML 路径使用 WindLInputSerializer::ReadYamlFile，
 *       否则使用 ReadTextFile。读取完成后调用 ResolvePaths 将文件内部相对路径
 *       解析为绝对路径，并调用 ApplyDerivedFields 计算派生字段
 * @code
 * auto input = ReadWindLInput("project.qwd");
 * @endcode
 */
...
/**
 * @brief 将 WindLInput 主输入数据写入文件
 * @param input 要写入的 WindLInput 数据，包含所有风场计算参数
 ...
 * @code
 * WriteWindLInput(input, "output.qwd", "template.qwd");
 * @endcode
 */
...
/**
 * @brief 转换 WindLInput 主输入文件格式
 * @param inputPath 输入文件路径（支持 .qwd 文本格式和 .yml/.yaml YAML 格式）
 * @param outputPath 输出文件路径
 * @param templatePath 文本输出模板路径（可选，仅对非 YAML 输出有效）
 * @note 内部调用 ReadWindLInput → WriteWindLInput，支持 .qwd ↔ .yml 互转
 * @code
 * ConvertWindLInput("input.qwd", "output.yml");
 * @endcode
 */
