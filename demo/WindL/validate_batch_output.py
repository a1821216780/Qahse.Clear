#!/usr/bin/env python3
"""Validate WindL batch .sum output against IEC 61400-1 Ed.3 formulas.
...
OUTPUT_DIR = Path(r"D:\WindL_SmokeTest\output_IEC3_1")
PLOT_DIR = Path(r"D:\WindL_SmokeTest\validation_plots")
REPORT_PATH = Path(r"D:\WindL_SmokeTest\validation_report.txt")
...
def parse_sum_file(path: Path) -> dict | None:
    """Parse a WindL .sum file; returns dict or None on failure."""
    try:
...
def validate_case(rec: dict) -> dict:
    ...
    def _ratio_checks(exp_su: float):
        ...
        These use the DERIVED SigmaU/V/W (what WindL intends to produce), not the
...
def generate_report(cases: list[dict]) -> str:
    ...
    lines.append(sep)
    lines.append("WindL Batch Output — IEC 61400-1 Validation Report")
    lines.append(sep)
