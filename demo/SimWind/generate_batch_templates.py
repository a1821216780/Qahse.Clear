...
def build_generic_rows() -> tuple[list[str], list[list[str]]]:
    ...
    def base(bundle: str, note: str, speed: float = 11.4) -> dict[str, object]:
        return {
            ...
            "Override.WrBlwnd": True,
            "Override.WrTrwnd": False,
            "Meta.Project": "SimWind batch template",
            "Meta.Turbine": NREL_5MW["Turbine"],
...
def fill_readme(ws, title: str, structure: str, total_cases: int) -> None:
    lines = [
        ...
        "",
        "Batch usage:",
        "  1. Open Qahse_SimWind_Batch_Template.qwd or your own Mode=1/BATCH template.",
        "  2. Point BatchExcel to this workbook and BatchSheet to Cases.",
        "  3. Rows use Override.* for SimWindInput overrides and Meta.* for record-only offshore metadata.",
        "  4. Current SimWind offshore scope is wind-field generation only; Meta.* wave/current/platform fields do not enter SimWind physics.",
        "",
...
def fill_catalog(ws, headers: Iterable[str]) -> None:
    rows = [
        ...
        ("Override.*", "Field override applied to SimWindInput before generation."),
