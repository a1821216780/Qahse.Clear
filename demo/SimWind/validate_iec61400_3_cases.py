#!/usr/bin/env python3
"""Certification-style validation for SimWind IEC wind conditions and spectra.
...
def qwd_text(case: Case, cfg: CampaignConfig, output_dir: Path) -> str:
    ...
    lines = [f"-- Auto-generated SimWind IEC certification case: {case.name}"]
...
def write_markdown_report(summary: dict[str, Any], cfg: CampaignConfig, path: Path) -> None:
    lines: list[str] = []
    lines.append("# SimWind IEC 61400-3 Validation Report")
    lines.append("")
    lines.append("## Scope")
    lines.append("")
    lines.append("This report validates the IEC wind-condition cases exposed by SimWind and audits the built-in spectral models.")
    lines.append("")
    ...
    lines.append("- The analytical references used here follow the SimWind implementation, the local TurbSim reference source, and the OpenFAST/TurbSim user guide.")
    lines.append("")
    ...
    lines.append("- Local TurbSim reference source: `F:\\HawtC3\\docs\\simwind\\ref\\Source\\VelocitySpectra.f90`, `TSsubs.f90`")
    lines.append("- Local Bladed format reference: `F:\\HawtC3\\docs\\simwind\\ref\\Bladed wind file format.md`")
    lines.append("")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
