...
def validate_api_rejection(exe: Path, template_qwd: Path, output_dir: Path) -> dict[str, Any]:
    ...
    if "GENERAL CohMod2" in text:
        ...
        text += "\nAPI CohMod2\n"
    bad_qwd = output_dir / "Qahse_SimWind_API_Invalid_DEMO.qwd"
...
def main() -> int:
    ...
    cases = [
        Phase2Case("UserSpectraScaled", "Qahse_SimWind_User_Spectra_Scaled_DEMO.qwd", False, validate_user_spectra),
        Phase2Case("USRVKM", "Qahse_SimWind_USRVKM_DEMO.qwd", True, validate_usrvkm),
        Phase2Case("IecKaimalStability", "Qahse_SimWind_IEC_Kaimal_Stability_DEMO.qwd", True, validate_stability),
        Phase2Case("MannReynoldsStress", "Qahse_SimWind_Mann_ReynoldsStress_DEMO.qwd", False, validate_mann_reynolds),
    ]

    reports = [run_case(case, exe, plot_root, args.skip_run) for case in cases]
    api_check = validate_api_rejection(exe, SCRIPT_DIR / "Qahse_SimWind_IEC_Kaimal_Stability_DEMO.qwd", artifact_dir)
