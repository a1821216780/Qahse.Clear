...
#include "../../src/SiMwind/Batch/SimWindBatch.hpp"
#include "../../src/SiMwind/IO/SimWind_IO_Subs.hpp"
...
public:
	ScopedBatchRoot()
		: root_(std::filesystem::temp_directory_path() / "Qahse_Clear_SimWindBatchTests")
	{
...
SimWindInput BatchTemplateInput(const ScopedBatchRoot &root)
{
	SimWindInput input;
                        ...
                        const std::vector<std::vector<std::string>> &rows)
{
	...
	excel.WCellValue<std::string>("README", "SimWind batch template", 0, 0);
...
TEST(WindL_Batch, ValidateOnlyReportsInvalidCaseButKeepsValidCase)
{
	...
	WriteSimWindInput(input, qwdPath.string());

	const auto result = SimWindBatch::RunFromFile(qwdPath.string(), "");
...
TEST(WindL_Batch, UnsupportedOverrideColumnFailsWorkbookValidation)
{
	...
	EXPECT_THROW((void)SimWindBatch::RunFromFile(qwdPath.string(), ""), std::runtime_error);
}
