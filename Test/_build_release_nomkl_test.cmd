...
echo ================================================================
echo   WindL Unit Tests — Release Build (without MKL)
echo   %DATE% %TIME%
echo ================================================================
...
if not exist "build\test\release_obj" mkdir "build\test\release_obj"

:: Step 3: Compile Release (/MD)
echo.
echo [BUILD] Compiling UnitTests_release_nomkl.exe ...
cl.exe /EHsc /std:c++20 /Zc:__cplusplus /MD /O2 /Ob2 /Ot /Oi /GL /Gy ^
  ...
  /Fe"build\test\UnitTests_release_nomkl.exe" ^
  /Fo"build\test\release_obj\\" ^
  /Fd"build\test\release_obj\vc143_release.pdb" ^
  ...
  > "build\test\release_nomkl_build.log" 2>&1
...
if %BUILD_RESULT% EQU 0 (
    ...
    echo ================================================================
    echo.
    echo [RUN] Executing WindL release tests without MKL...
    echo.
    "build\test\UnitTests_release_nomkl.exe" --gtest_filter="WindL*"
    set TEST_RESULT=%ERRORLEVEL%
    echo.
    echo [RUN] All release tests (regression)...
    echo.
    "build\test\UnitTests_release_nomkl.exe"
    set ALL_RESULT=%ERRORLEVEL%
    echo.
    echo Test exit code: WindL=%TEST_RESULT% All=%ALL_RESULT%
) else (
    echo   BUILD FAILED (exit code: %BUILD_RESULT%)
    echo ================================================================
    echo.
    echo First 30 errors:
    findstr /C:"error" "build\test\release_nomkl_build.log" | findstr /V /C:"Command line warning" 2>nul
)
